<template>
    <div>
        <div class="comments_wrapper" v-for="(comment, key) in post.comments" :key="key"
      ...
      .then((data) => {
        ...
        this.comments.forEach((post) => {
          post.showStatus = "Show More"
           if (this.likedcomments.includes(post.id)) {
        ...
        this.commentPage.push(this.comments.slice(0, 10))
      })
      .catch((err) => {});
