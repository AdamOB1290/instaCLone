<template>
    <div
        key="feedKey"
        :class="`h-100 mt-1 ${sessionUser.followed.length != 0 ? 'mb-5' : ''}`"
    >
        <div
            v-if="sessionUser.followed.length != 0"
            class="feed_content flex justify-between pb-12 sm:pb-5 text-xs mx-auto sm:w-3/4 lg:text-sm  "
        >
            <div class="w-full md:w-8/12 lg:w-7/12 xl:w-8/12 ">
                ...
                <div
                    class="feed_wrapper"
                    v-for="(page, key) in postFeed"
                    :key="key"
                >
                    <div
                        class="post_wrapper pt-2"
                        v-for="(post, key) in page"
                        :key="key"
                    >
                        <div class="card rounded-0">
                            <div
                                class="card-head d-flex align-items-center border-down py-1"
                            >
                                <img
                                    @click="goToProfile"
                                    :data-userId="post.user.id"
                                    class="pfp  rounded-circle mr-2"
                                    :src="post.user.pfp"
                                />
                                <span
                                    @click="goToProfile"
                                ...
                                >
                                <i
                                    class="fas fa-ellipsis-h text-secondary ml-auto mr-2 pt-2"
                                    v-b-modal="'my_postModal' + post.id"
                                ></i>
                                <b-modal
                                    modal-class="settings_Modal"
                                    :id="'my_postModal' + post.id"
                                    :ref="'my_postModal' + post.id"
                                    hide-header
                                    hide-footer
                                    centered
                                >
                                    <button
                                        :id="'postEditId' + post.id"
                                        :data-postId="post.id"
                                        @click="editPostDescription"
                                        class="w-100 settings_btn px-5 py-2"
                                    >
                                        Edit
                                    </button>
                                    <button
                                        :id="'postShareStoryId' + post.id"
                                        :data-postId="post.id"
                                        @click="shareStory"
                                        class="w-100 settings_btn px-5 py-2"
                                    >
                                    ...
                                    <button
                                        :id="'postDeleteId' + post.id"
                                        :data-postId="post.id"
                                        @click="deletePost"
                                        class="w-100 settings_btn text-danger px-5 py-2 border-0"
                                    >
                                        Delete
                                    </button>
                                </b-modal>
                            ...
                            <div class="card-body">
                                ...
                                <div class="px-2">
                                    <div
                                        class="interaction_buttons pt-2 d-flex"
                                    >
                                        <!-- like icon -->
                                        <div class="w-10">
                                            <svg
                                        ...
                                        <!-- comment icon -->
                                        <router-link
                                            @click="jelloAnimation"
                                            :to="'post/' + post.id"
                                        >
                                            <svg
                                                :id="'postCommentId' + post.id"
                                                :data-postId="post.id"
                                                aria-label="Comment"
                                                class="_8-yf5 mr-3"
                                                fill="#262626"
                                        ...
                                        <b-modal
                                            :id="'my_sharePostModal' + post.id"
                                            :ref="'my_sharePostModal' + post.id"
                                            modal-class="sharePost_Modal"
                                            hide-header
                                            hide-footer
                                        >
                                            <ul
                                                class="sharePostUl position-relative"
                                            >
                                                <span
                                                    @click="
                                                        $bvModal.hide(
                                    ...
                                    <div class="card-text position-relative">
                                        <span
                                            v-if="post.editState"
                                            :id="'cancelPostEditId' + post.id"
                                            :data-postId="post.id"
                                            @click="cancelEditPostDescription"
                                            class="cursor-pointer cancel_edit post_cancel text-danger"
                                            >Cancel</span
                                        >
                                        ...
                                        <span
                                            :ref="'showMoreLess' + post.id"
                                            class="show_hide"
                                            @click="showHideDescription"
                                            >{{ post.showStatus }}</span
                                        >
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card-footer px-2 py-0 bg-white">
                                ...
                                <div
                                    ...
                                    v-for="(comment,
                                    key) in post.comments.slice(0, 2)"
                                    :key="key"
                                ...
                                >
                                    <div class="d-flex position-relative">
                                        ...
                                        <!-- like icon -->
                                        <svg
                                            :id="'commentLikeId' + comment.id"
                                            :data-commentId="comment.id"
                                            @click="likeUnlikeComments"
                                            :fill="comment.likeColor"
                                            class="comment_like_icon postfeed"
                                            aria-label="Like"
                                            viewBox="0 0 48 48"
                                        >
                    ...
                    >
                        <b-form-group>
                            <div class="form-group d-flex flex-column">
                                ...
                                />
                                <cld-image
                                    v-if="postForm.resourceType == 'image'"
                                    dpr="auto"
                                    responsive="width"
                                    width="auto"
                                    crop="scale"
                                    :publicId="postForm.publicId"
                                    class="my-2"
                                >
                                    <cld-transformation
                                        quality="auto"
                                ...
                                <video
                                    class="video img ml-auto bg-dark"
                                    v-else-if="postForm.resourceType == 'video'"
                                    controls
                                    muted
                                >
                                ...
                                <div class="px-2">
                                    ...
                                    ></b-form-select>
                                    <b-form-textarea
                                        v-model="postForm.postCaption"
                                        name="description"
                                        placeholder="Add a caption ..."
                                        cols="30"
                                        rows="1"
                                        max-rows="10"
                                        class="form-control addPostTxt mb-2"
                                    ></b-form-textarea>
                                </div>
                                <button
                                    type="submit"
            ...
            <div
                class=" search_component_wrapper hidden md:inline-block md:w-4/12 lg:w-5/12 xl:w-4/12"
            >
                <h1 class="text-gray-800 text-xs mt-2 text-center">
                    Suggestions for you :
                </h1>
        ...
        var widget = cloudinary.createUploadWidget(
            ...
            (error, result) => {
                ...
                if (typeof result.info.files != "undefined") {
                    this.postForm.publicId =
                        result.info.files[0].uploadInfo.public_id;
                    this.postForm.resourceType =
                        result.info.files[0].uploadInfo.resource_type;
                            ...
                            if (this.nodeType === Node.ELEMENT_NODE) {
                                ...
                                void (function replaceNode(text) {
                                    ...
                                    text.nodeValue.replace(
                                        /[\s\S]/gm,
                                        function wrapCharacter(character) {
    ...
    methods: {
                ...
                axios
                    ...
                    .then(response => {
                        if (
                            ...
                            $("#" + postLikeId)[0].attributes[2].nodeValue =
                                "#262626";
                            $(
                                "#" + postLikeId
                            )[0].firstChild.attributes[0].nodeValue =
                                "M34.6 6.1c5.7 0 10.4 5.2 10.4 11.5 0 6.8-5.9 11-11.5 16S25 41.3 24 41.9c-1.1-.7-4.7-4-9.5-8.3-5.7-5-11.5-9.2-11.5-16C3 11.3 7.7 6.1 13.4 6.1c4.2 0 6.5 2 8.1 4.3 1.9 2.6 2.2 3.9 2.5 3.9.3 0 .6-1.3 2.5-3.9 1.6-2.3 3.9-4.3 8.1-4.3m0-3c-4.5 0-7.9 1.8-10.6 5.6-2.7-3.7-6.1-5.5-10.6-5.5C6 3.1 0 9.6 0 17.6c0 7.3 5.4 12 10.6 16.5.6.5 1.3 1.1 1.9 1.7l2.3 2c4.4 3.9 6.6 5.9 7.6 6.5.5.3 1.1.5 1.6.5.6 0 1.1-.2 1.6-.5 1-.6 2.8-2.2 7.8-6.8l2-1.8c.7-.6 1.3-1.2 2-1.7C42.7 29.6 48 25 48 17.6c0-8-6-14.5-13.4-14.5z";
        ...
        likeUnlikeComments(event) {
            ...
            if (typeof $(event.target).attr("id") == "undefined") {
                ...
                dataPostId = $(event.target.parentElement).attr("data-postId");
                dataCommentId = $(event.target.parentElement).attr(
                    "data-commentId"
                );
            } else {
                target = $(event.target);
                ...
                axios
                    ...
                    .then(response => {
                        if (
                            ...
                            $("#" + commentLikeId)[0].attributes[2].nodeValue =
                                "#262626";
                            $(
                                "#" + commentLikeId
                            )[0].firstChild.attributes[0].nodeValue =
                                    ...
                                    if (post.comments.length > 0) {
                                        post.comments.forEach(comment => {
                                            if (comment.id == dataCommentId) {
                                                let index = comment.likes.indexOf(
                                                    this.sessionUser.id
                                                );
                    ...
                    .then(response => {
                        if (
                            ...
                            $("#" + commentLikeId)[0].attributes[2].nodeValue =
                                "#ed4956";
                            if (
                                target.hasClass("reduce_comment_like_animation")
                            ) {
                                target.removeClass(
                                    "reduce_comment_like_animation"
                                );
                            }
                            target.addClass(
                                "animate__animated animate__bounceIn"
                ...
                axios
                    ...
                    .then(response => {
                        if (
                            ...
                            $("#" + postSaveId)[0].attributes[3].nodeValue =
                                "#262626";
                            $(
                                "#" + postSaveId
                            )[0].firstChild.attributes[0].nodeValue =
        ...
        blobToFile(theBlob, fileName) {
            ...
            var theFile = new File([theBlob], fileName, {
        ...
        sendAnimation(event) {
            $(event.target).addClass(
                "animate__animated  animate__fadeOutTopRigh animate__slow"
            );
        }
    },
